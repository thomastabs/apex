...
class TestProxyFigmaCatchAll:
    ...
    def test_oauth_token_forwarded_as_bearer(self, client):
        # A token without the figd_ PAT prefix is an OAuth access token → it must
        # go upstream as Authorization: Bearer, not X-Figma-Token.
        upstream = _mock_upstream(200, {"ok": True})
        patcher, mock_http = _patch_client(upstream)
        with patcher:
            resp = client.get("/api/design/figma/me", headers={"X-Figma-Token": "oauth-access-tok"})
        assert resp.status_code == 200
        hdrs = mock_http.request.call_args.kwargs["headers"]
        assert hdrs.get("Authorization") == "Bearer oauth-access-tok"
        assert "X-Figma-Token" not in hdrs


class TestFigmaAuthHeaders:
    def test_pat_uses_x_figma_token(self):
        from backend.app.services.figma_fetch import figma_auth_headers

        assert figma_auth_headers("figd_abc") == {"X-Figma-Token": "figd_abc"}

    def test_oauth_token_uses_bearer(self):
        from backend.app.services.figma_fetch import figma_auth_headers

        assert figma_auth_headers("opaque-oauth") == {"Authorization": "Bearer opaque-oauth"}

    def test_token_is_trimmed(self):
        from backend.app.services.figma_fetch import figma_auth_headers

        assert figma_auth_headers("  figd_x  ") == {"X-Figma-Token": "figd_x"}


class TestFigmaOAuth:
    """OAuth routes — operator-gated, frontend-stored token posture."""

    def _enable(self, monkeypatch):
        monkeypatch.setenv("FIGMA_OAUTH_CLIENT_ID", "cid123")
        monkeypatch.setenv("FIGMA_OAUTH_CLIENT_SECRET", "secret456")
        monkeypatch.setenv("FIGMA_OAUTH_REDIRECT_URI", "https://apex-bolt.com/figma/callback")

    def test_config_disabled_by_default(self, client, monkeypatch):
        monkeypatch.delenv("FIGMA_OAUTH_CLIENT_ID", raising=False)
        monkeypatch.delenv("FIGMA_OAUTH_CLIENT_SECRET", raising=False)
        monkeypatch.delenv("FIGMA_OAUTH_REDIRECT_URI", raising=False)
        assert client.get("/api/design/figma/oauth/config").json() == {"enabled": False}

    def test_config_enabled_when_env_set(self, client, monkeypatch):
        self._enable(monkeypatch)
        assert client.get("/api/design/figma/oauth/config").json() == {"enabled": True}

    def test_authorize_url_built(self, client, monkeypatch):
        self._enable(monkeypatch)
        monkeypatch.setenv("FIGMA_OAUTH_SCOPES", "file_content:read,projects:read")
        url = client.get("/api/design/figma/oauth/authorize-url?state=xyz").json()["url"]
        assert url.startswith("https://www.figma.com/oauth?")
        assert "client_id=cid123" in url
        assert "state=xyz" in url
        assert "response_type=code" in url
        assert "scope=file_content%3Aread%2Cprojects%3Aread" in url

    def test_authorize_url_404_when_disabled(self, client, monkeypatch):
        monkeypatch.delenv("FIGMA_OAUTH_CLIENT_ID", raising=False)
        assert client.get("/api/design/figma/oauth/authorize-url?state=xyz").status_code == 404

    def test_authorize_url_requires_state(self, client, monkeypatch):
        self._enable(monkeypatch)
        assert client.get("/api/design/figma/oauth/authorize-url?state=").status_code == 400

    def test_token_exchange_success(self, client, monkeypatch):
        self._enable(monkeypatch)
        resp = _mock_upstream(200, {"access_token": "atok", "refresh_token": "rtok", "expires_in": 7776000})
        mock_client = MagicMock()
        mock_client.__enter__ = MagicMock(return_value=mock_client)
        mock_client.__exit__ = MagicMock(return_value=False)
        mock_client.request = MagicMock(return_value=resp)
        # _mock_upstream gives .content/.status_code/.headers; add .json()
        resp.json = MagicMock(return_value={"access_token": "atok", "refresh_token": "rtok", "expires_in": 7776000})
        with patch("backend.app.api.figma_oauth.httpx.Client", return_value=mock_client):
            out = client.post("/api/design/figma/oauth/token", json={"code": "authcode"})
        assert out.status_code == 200
        assert out.json() == {"access_token": "atok", "refresh_token": "rtok", "expires_in": 7776000}
        # client credentials go in the Basic auth header, not the body
        sent = mock_client.request.call_args
        assert sent.kwargs["headers"]["Authorization"].startswith("Basic ")
        assert sent.kwargs["data"]["grant_type"] == "authorization_code"
        assert sent.kwargs["data"]["code"] == "authcode"

    def test_token_exchange_rejected_is_400(self, client, monkeypatch):
        self._enable(monkeypatch)
        resp = _mock_upstream(400, {"error": "invalid_grant"})
        mock_client = MagicMock()
        mock_client.__enter__ = MagicMock(return_value=mock_client)
        mock_client.__exit__ = MagicMock(return_value=False)
        mock_client.request = MagicMock(return_value=resp)
        with patch("backend.app.api.figma_oauth.httpx.Client", return_value=mock_client):
            out = client.post("/api/design/figma/oauth/token", json={"code": "bad"})
        assert out.status_code == 400

    def test_token_exchange_404_when_disabled(self, client, monkeypatch):
        monkeypatch.delenv("FIGMA_OAUTH_CLIENT_ID", raising=False)
        out = client.post("/api/design/figma/oauth/token", json={"code": "x"})
