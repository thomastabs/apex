...
class TestSaveAndLoad:
    ...
    def test_personal_key_always_active_alongside_a_system_key(self, monkeypatch):
        # A saved personal key must be returned by load_keys unconditionally —
        # there is no "prefer system" switch; the caller (deps.py) is the one
        # that decides to fall back to the env var, and only when nothing here
        # is returned for that provider.
        monkeypatch.setenv("OPENAI_API_KEY", "sk-system-env-key")
        ai_key_store.save_key("api_taiga_io", "42", "openai", "sk-personal-key")
        assert ai_key_store.load_keys("api_taiga_io", "42") == {"openai": "sk-personal-key"}
...
class TestLegacyWrapperShape:
    """A brief earlier iteration of this feature wrapped each account's entry
    as {"keys": {...}, "prefer_system": [...]}. That shape must still read
    back correctly (as if nothing were ever marked prefer_system — a saved
    key is unconditionally active now)."""

    def test_saved_providers_reads_wrapper_entry(self):
        ai_key_store._write_all("api_taiga_io", {"42": {"keys": {"openai": "sometoken"}, "prefer_system": ["openai"]}})
        assert ai_key_store.saved_providers("api_taiga_io", "42") == ["openai"]

    def test_load_keys_decrypts_wrapper_entry_regardless_of_prefer_system(self):
        ai_key_store.save_key("api_taiga_io", "42", "openai", "sk-legacy")
        raw = ai_key_store._read_all("api_taiga_io")
        wrapped = {"42": {"keys": {"openai": raw["42"]["openai"]}, "prefer_system": ["openai"]}}
