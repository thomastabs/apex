...
- **ER Diagram** — interactive React Flow entity-relationship diagram generated automatically when the Data Model section completes; entities shown as cards (primary keys in amber, foreign keys in blue, with field types); dagre auto-layout on first render; drag nodes to rearrange; layout persisted to `diagram-er.json`; resizable canvas; regenerate on demand
...
| `diagram-er.json` | React Flow ER diagram nodes and edges generated by Phase 2 (includes saved layout positions) |
