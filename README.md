...
- **Figma integration** — link a Figma file via a Personal Access Token (or **Connect with Figma** via OAuth, when the deployment configures it — see below) + a file **or project** URL (a project URL lists the project's files to pick from — needs the `projects:read` token scope); **Sync Context** pulls the file's pages, top-level frame (screen) names, prototype flows, and comments into `figma-context.md`, which is injected into Phase 1 story generation and Phase 2 design prompts. In Phase 1 you can also select frames and generate user stories directly from them (the "tasks from a UI perspective" loop). Per-story frames can be linked in the board's story dialog (deep link + thumbnail), and the design can drive the Phase 2 screen-flow diagram directly. Figma REST calls are routed through a backend proxy (`/api/design/figma/*`, SSRF-guarded to `api.figma.com`) because the Figma API has no permissive CORS; the token is never persisted
  ...
  - **OAuth ("Connect with Figma")** — *optional, operator-gated.* When the deployment registers a Figma OAuth app and sets `FIGMA_OAUTH_CLIENT_ID` / `FIGMA_OAUTH_CLIENT_SECRET` / `FIGMA_OAUTH_REDIRECT_URI` (redirect = `<frontend-origin>/figma/callback`; scopes default to `file_content:read,file_comments:read,projects:read`, override with `FIGMA_OAUTH_SCOPES`), the sidebar shows a **Connect with Figma** button — users sign in instead of pasting a PAT. The backend holds the client secret and performs only the authorization-code exchange (`/api/design/figma/oauth/*`); the access token is handed to the browser and stored in `sessionStorage` exactly like a PAT (cleared on tab close, never persisted server-side, no cross-session refresh). The proxy detects the token kind (PATs start `figd_` → `X-Figma-Token`; OAuth tokens → `Authorization: Bearer`), so everything downstream is unchanged. The PAT path remains for deployments that don't register an app; when unconfigured, the button is hidden
...
| `backend/app/api/figma_proxy.py` | FastAPI reverse proxy for the Figma REST API (host-locked to `api.figma.com`, DNS-rebinding pinned; forwards a PAT as `X-Figma-Token` or an OAuth token as `Authorization: Bearer`, picked by token prefix) |
| `backend/app/api/figma_oauth.py` | Optional, operator-gated Figma OAuth — `config` / `authorize-url` / `token` exchange (holds the client secret; token returned to the browser, nothing stored server-side) |
...
# Optional. Figma OAuth ("Connect with Figma"). UNSET = button hidden, PAT-only
# (default). Set all three to enable: register a Figma OAuth app, set the redirect
# to <frontend-origin>/figma/callback. Scopes default to
# file_content:read,file_comments:read,projects:read. The backend holds the secret
# and only exchanges the code; the access token is stored client-side like a PAT.
# FIGMA_OAUTH_CLIENT_ID=
# FIGMA_OAUTH_CLIENT_SECRET=
# FIGMA_OAUTH_REDIRECT_URI=https://apex-bolt.com/figma/callback
# FIGMA_OAUTH_SCOPES=file_content:read,file_comments:read,projects:read
