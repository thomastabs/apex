...
**Semantic versioning.** Every lockable artifact carries a real version number so humans can consult "has this actually changed since I last read it" without diffing markdown by eye: `0.0.0` while still a pre-lock draft, `1.0.0` the moment it locks, `+1` MAJOR on every post-lock amendment (an amendment is already, by the mechanic above, a breaking change — every affected downstream story gets flagged), and `+1` **MINOR** for a [Design Delta](#phase-2--design) append — the one edit the system can *prove* is additive (nothing existing changed, so nothing downstream drifts). There is still no PATCH: the data model has no signal for a cosmetic edit, and faking one would be noise. Versions persist in `spec-versions.json` and show as a badge next to each locked file in the sidebar's Active Context panel.
...
- **GitHub PAT / Figma token — encrypted at rest, opt-in persistence.** Both are used browser-direct (GitHub/Figma calls never route through the backend proxy) but, unlike the PM token, users asked to not have to re-enter them every session. Saving one now also encrypts and persists it per-instance (`AI_KEY_ENCRYPTION_SECRET`-derived Fernet cipher — the same guarantee `ai_key_store.py` gives AI provider keys), and a dedicated reveal endpoint (`GET /api/workspace/github-pat` / `/figma-token`, never the general `/config` response) restores it into the browser session on load. This is a deliberate tradeoff: unlike AI keys, which never leave the backend, these two must reach the browser to be used, so the backend does see the raw credential during save/restore — accepted for the convenience, encrypted at rest to bound a File Share compromise.
