...
- Generate a **design bundle** in a 3-section AI cascade (each section uses previous output as context for consistency), plus an independent 4th step:
...
**Stable spec ids.** Individual requirements — not just stories/epics — carry a stable, AI-assigned id, so specs are addressable rather than matched by prose: `EP-N` endpoints, `ENT-N` data-model entities, `SCR-N` UX Brief screens (all project-global, `## Assumptions`-style `{ID}: text` bullet convention shared across the three), and `SC-N` Gherkin scenarios (scoped per story, like task ids — rendered as a `@SC-N` tag above the scenario, same convention `constraints.md`'s `NFR-N` ids already used). A [Design Delta](#phase-2--design) continues numbering from the locked design's max id rather than restarting. `spec-index.json` is a flat `{id: {kind, label, assumptions}}` registry rebuilt automatically on every spec write — a foundation for addressing/indexing individual requirements rather than whole files.
