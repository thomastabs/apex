...
The right sidebar's Active Context panel shows the Markdown files Apex uses to ground AI calls, with size/budget signals and phase-specific visibility. From there, a Taiga project can publish those context files into deterministic `apex-*` Taiga Wiki pages, or pull matching wiki pages back into Apex when documentation was edited in Taiga. Taiga-only custom Wiki pages are kept behind a collapsed disclosure, show only non-empty pages by default, and must be selected explicitly before Apex imports them as AI grounding files. Publishing also bookmarks each page (Taiga's `WikiLink` resource) so it shows up under the project's own Wiki-home **BOOKMARKS** list in Taiga itself — without this, a newly published page is real but invisible unless you already know its slug or dig through Apex's full custom-page listing.
...
- **Intake** of post-deployment feedback from a manual in-app form, **GitHub Issues**, **Taiga Issues**, **Plane** work items, and unresolved **Figma comments** (read-only import; net-new or linked to a deployed story). PM-issue imports (Taiga/Plane) carry the upstream issue's own report timestamp as `detected_at` and default the story link to a word-overlap suggestion — human-reviewable in a picker before Import — since a linked, dated item is what feeds the **AI Defect Escape Rate** below. They also carry the PM tool's own **severity/priority** label (Taiga's project-configurable severity name, or Plane's priority) — shown next to the AI's severity_hint in Severity Routing as an independent ground-truth signal, never auto-applied
...
**GitHub webhook — auto context resync + regression scan on push:** the regression scan above is on-demand (a human clicks "Scan for regressions"), and Sync Context is normally a button click too. A repo can instead register `POST /api/webhooks/github/{instance_id}/{project_id}` as its push webhook (payload URL + HMAC secret generated per instance, shown in the sidebar GitHub panel under "Auto regression scan & context sync on push") so every push closes both loops itself: **every push** re-clones and repacks `github-context.md` (the same server-side clone+repomix pipeline as a manual Sync), and the handler additionally matches the push's touched files against each story's saved developer-pack "Files to Change" (pure string overlap, no AI) to re-verify conformance for just the affected stories — both run as background tasks (the webhook response returns immediately — GitHub's 10s delivery timeout would otherwise kill either job mid-flight). Both share one 5-minute per-project cooldown gate (repacking runs on every push within the window regardless of whether any story matched; the regression scan additionally requires a match) so a burst of pushes can't trigger unbounded AI/clone spend. Requests are rejected unless the `X-Hub-Signature-256` HMAC matches the per-instance secret (`backend/app/api/github_webhook.py`) — unauthenticated by necessity (GitHub can't send a bearer token) but never trusted without it. **The same endpoint also handles `workflow_run` events**, which is what lets a Phase 5 GitHub Actions deployment (see above) mark a story `deployed` automatically the moment a dispatched run completes — this is a second, independent event type on the same webhook, not something `push` alone covers. Registering the webhook for `push` only (GitHub's own "Just the push event" default) means Phase 5 dispatches will run and even succeed on GitHub's side, but Apex never learns they finished until someone clicks "Sync run" by hand; select "Let me select individual events" and check both **Pushes** and **Workflow runs** when creating the webhook to get both behaviors.

### Specs as a living document

Spec files are editable at any time, locked or not — "locked" gates which AI workflows are eligible for the next phase, it never freezes the file. Editing a **locked** spec artifact (e.g. `functional-spec.md` after `gherkin_locked`, `technical-spec.md`/`design-bundle.md`/`constraints.md` after `design_locked`) via the sidebar is never silent, though: the edit is logged to `amendments.md` as a dated **amendment** naming which stories were affected. That's an audit trail, not a required review — there's no flag, no badge, no acknowledge step. Specs work as an always-current library of guidelines, requirements, plans, and tests; Phase 3–6 prompts always inject whatever the file currently says.

**Attribution granularity.** An earlier version of this audit trail named *every* story at/after the file's lock on any edit — for `functional-spec.md` specifically, that's fixed: each `@SC-n` Gherkin scenario already carries an exact owning story id (`spec-index.json`), so a diff against the pre-edit content narrows `amendments.md`'s affected-stories line to only the stories whose own scenario actually changed (an edit outside any tagged scenario — a heading, a story description — falls back to the full set rather than under-reporting). `technical-spec.md`/`design-bundle.md`/`constraints.md`/`runtime-spec.md` still name every story at/after their lock: nothing in the system attributes an individual endpoint/entity/screen bullet to a specific story the way scenarios are attributed, and fabricating that ownership here would trade one wrong precision for another. This is a real, if partial, gap the technical-spec.md side of this construct is known to have — see `docs/thesis` Chapter 7's account of why it was originally built project-wide, then reverted, for that file.
...
- **Context Traceability Rate** — % of deployed stories whose context chain actually *resolves*, not just exists: every artifact present (Gherkin + test plan + infra delta + complete matrix + deployment-log entry) AND no known break in the chain — no unresolved `trace_flag` (a downstream failure pointed back at an earlier phase), no unacknowledged `conformance_regressed` (a later code change broke what used to match the locked spec). A story with every file present but a live regression counts as unresolved, not complete — see `AnalyticsService._chain_resolved`'s docstring for the full three-leg definition and what it deliberately doesn't attempt (per-endpoint attribution into technical-spec.md — see the Spec Drift attribution-granularity note above)
...
- **Fix-Bolt defect proxy** — total/avg Fix-Bolt triggers per story: a *pre-deploy* QA-catch rate (each failed QA gate increments the counter), not the Defect Escape Rate itself
- **AI Defect Escape Rate** — % of deployed stories with a linked maintenance item, **AI/human-classified as a bug**, detected *after* the story's own deployed timestamp. Still a proxy, not production telemetry — a defect nobody reports, reports without linking the story, or that never gets triaged stays invisible — but unlike the Fix-Bolt proxy above it counts only genuine post-deploy escapes, not QA catches. The bug-classification gate matters most on Plane: it has no dedicated issue-tracker resource (every work item doubles as story/task/issue), so counting unclassified imports would count ordinary new work as "defects" — requiring Path B classification first is what keeps the metric meaningful on any of the three PM tools. Sync + link an issue in **Phase 6 → Maintenance**, then classify it, to feed this metric; see [Phase 6 · Maintenance & Traceability](#phase-6--maintenance--traceability)
...
- **Loud by default, silence is opt-in.** `frontend/app/providers.tsx` installs a `QueryCache.onError` and a `MutationCache.onError` on the app's QueryClient (`createAppQueryClient`, exported so tests exercise the real client instead of a bare one that silently drops failures). Both funnel into `notifyError` (`frontend/lib/error-toast.ts`), which classifies the failure through `classifyError` (`frontend/lib/errors.ts`, an `ErrorKind` union) and renders a translated title, the server's own detail, and an actionable hint via [sonner](https://sonner.emilkowal.ski/).
- **The convention.** A query or mutation names itself with `meta: { errorLabel: "op.<name>" }` (shown as "<label> failed"), or opts out entirely with `meta: { toastSilent: true }` for genuinely optional/background calls. A per-hook or call-site `onError` should not be added just to fire a toast - it double-reports (the global net already does it) and, at a call site, TanStack drops the callback if the component unmounts before the promise resolves. A call-site `onError` is only for non-toast work: optimistic rollback, clearing local state.
- **Typed transport errors.** `ApiError` (`.status`/`.detail`/`.code`), `ApiTimeoutError`, and `ApiNetworkError` live in `frontend/lib/api/client.ts`. A client-side deadline is distinct from a user cancel: `useCancellableMutation` swallows only its own `cancel()`, and shows a long-running-operation loading toast after 8s so a multi-minute AI call doesn't read as a hang.
- **Backend error contract.** Every response uses `{"detail": ...}`, where `detail` is either a plain string or `{"code": ..., "message": ...}` for cases the client must distinguish (`ai_key_rejected`, `ai_model_rejected`, `ai_context_length`, `github_repo_not_found`, `workspace_busy`, …). `backend/app/main.py` registers handlers that normalise `RequestValidationError`, every `HTTPException`, and a distributed-lock `TimeoutError` (→ 503 `workspace_busy`) into that shape, and logs all 5xx; log verbosity is set via the `LOG_LEVEL` env var.
- **Shared AI error mapper.** `backend/app/api/ai_errors.py` (`handle_ai_error`) is the single AI-error → HTTP mapping used by all six phase routers and `workspace.py` - no router re-implements it. `AIRateLimitError` → 429, `AITimeoutError` → 504, `AIAuthError` → 401, `AIModelError` → 400, `AIContextLengthError` → 413, `AIContentFilterError`/`AIValidationError`/`AIError` → 502, and a missing API key (`EnvironmentError` from `check_api_key`) → 401.
- **Rate limiting.** Every 429 carries a `Retry-After` header and a `code` identifying which limiter fired (`backend/app/api/rate_limit.py`).
- **i18n and copy.** All toast strings - titles, hints, action labels - go through `frontend/lib/i18n/translations.ts` in EN and PT; no emoji appears in any user-facing string (severity is carried by the sonner variant, never by characters in the copy). `frontend/tests/translations-integrity.test.ts` enforces EN/PT key parity and the no-emoji rule.
...
Eight spec files against mocked backend and Taiga APIs. Five exercise one full phase flow each: `phase4-testing-flow` covers the pass path and the Fix-Bolt fail path; `phase5-deploy-flow` covers the routine-bypass deployment and the changes-flagged → pack → reject/revise → gate path. The three documented in detail below. The other three cover cross-cutting surfaces rather than a single phase: `analytics-dashboard` (governance metrics/funnel/cycle-time rendering, CSV export, and the failure-callout path when the summary endpoint errors), `autopilot-flow` (setup/running/paused/done states, Stop, and adding epic rows), and `figma-design-sync` (connecting a Figma file and syncing design-system context).
...
- AI errors map to distinct HTTP codes through the shared mapper in `backend/app/api/ai_errors.py` (`handle_ai_error`) - see [Error Surfacing](#error-surfacing) for the full mapping; do not re-implement it per router.
