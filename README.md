...
- **Figma-grounded decomposition** — when a Figma file is synced, the screens + screen flow from `figma-context.md` are injected, so UI tasks reference the real screens and navigation instead of inventing them (advisory; never adds work beyond the Gherkin). When the context was seeded by Autopilot it also carries the design system (component inventory, colour/text tokens)
...
- **Figma integration** — link a Figma file via a Personal Access Token + a file **or project** URL (a project URL lists the project's files to pick from — needs the `projects:read` token scope); **Sync Context** pulls the file's pages, top-level frame (screen) names, and prototype flows into `figma-context.md`, which is injected into Phase 1 story generation, Phase 2 design, **Phase 3 task decomposition + developer packs, and Phase 4 test plans**. (Sync is deliberately a **single** Figma call — screens + flows — because Figma's rate limit is cost-based and per-token; fanning out to the design-system style/component endpoints + comments made one Sync cost ~5 calls and routinely tripped a token-wide 429 that blocked even small files. Autopilot's server-side seeding still extracts the design-system tokens, which it can do once per run without the interactive re-Sync pressure.) In Phase 1 you can also select frames and generate user stories directly from them (the "tasks from a UI perspective" loop). Per-story frames can be linked in the board's story dialog (deep link + thumbnail), the design can drive the Phase 2 screen-flow diagram directly, and a linked frame is rendered into the matching Phase 3 developer pack for multimodal grounding. Figma REST calls are routed through a backend proxy (`/api/design/figma/*`, SSRF-guarded to `api.figma.com`) because the Figma API has no permissive CORS; the token is never persisted
...
| `frontend/lib/api/figma.ts` | Figma REST client (via the backend proxy) — file/frames/thumbnails/comments, design-token extraction, URL parsing, story↔frame matching, and context-markdown assembly |
...
| `figma-context.md` | File name, pages, top-level frame names, and prototype flows synced from a linked Figma file (interactive Sync is a single cost-bounded call — screens + flows; Autopilot seeding additionally extracts design-system tokens); injected into Phase 1 story generation, Phase 2 design, Phase 3 task decomposition + developer packs, and Phase 4 test plans |
