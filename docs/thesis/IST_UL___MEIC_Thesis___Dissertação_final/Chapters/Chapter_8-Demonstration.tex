...
\todo[inline]{Only one fieldwork setting remains (Outfolio; the partner organisation is recorded in \Cref{sec:demo_design} as abandoned). Describe Outfolio concretely beyond the origin above: its fixed scope (the epics and stories committed to in advance, referenced from \Cref{sec:demo_design} above), which hats the author wears solo across it, the existing tooling and process baseline it is built against. Establish the baseline concretely enough that a reader can judge what the framework changed. Since this is a solo project rather than a team, do not overstate team-composition detail that does not exist; state plainly that hats are worn by one person here, which is itself a case the framework's own "hats, not people" design in \Cref{sec:roles} is meant to cover.}
