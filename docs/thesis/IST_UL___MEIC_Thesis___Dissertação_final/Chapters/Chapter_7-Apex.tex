% #############################################################################
% This is Chapter 7 - Proposal Implementation
% !TEX root = ../main.tex
% #############################################################################
...
\fancychapter{Proposal Implementation}
\label{chap:apex}
...
\begin{figure}[p]
    ...
    \caption{Apex's system architecture: the frontend, the backend and the specification store. The figure is split across two pages for legibility; the point of it is the position of the two boundaries rather than the inventory of components, and this half shows the first, where all workflow state is written to a per-tenant, per-project specification store rather than to an application database. Continued in \Cref{fig:apex_architecture_b}.}
...
\begin{figure}[p]
    ...
    \caption{The same architecture, continuing \Cref{fig:apex_architecture}: the external services Apex integrates with. This half shows the second boundary the figure exists to make, that every outbound call to a user-configurable host passes through a server-side guarded proxy.}
...
The two proxies differ in kind, and \Cref{tab:apex_mapping} names both as proxies rather than presenting either as the construct itself: Bolt Cycle Time was one briefly and by accident, corrected once the substitution was noticed, while AI Defect Escape Rate is one by design from the outset. A substitution left unnamed is indistinguishable from the thing it substitutes for.
