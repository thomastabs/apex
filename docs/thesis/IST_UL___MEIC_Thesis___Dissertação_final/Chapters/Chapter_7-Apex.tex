...
A further, separate consultation is reported here for the same reason and under the same limits. An Agile Coach at a large organisation was given a live presentation of the framework followed by a demonstration of Apex applied to a concrete example. The reaction was that a tool which instantiates and applies the framework's roles and phases makes the framework tangible in a way a description alone does not, which is the demonstration argument \Cref{chap:demonstration} makes independently. The comparison offered spontaneously was against the rigid, closed-role structures typical of frameworks adopted inside large organisations, against which the framework's hats and its refusal to lock roles to named individuals were received as a genuine point of difference; the Mob Elaboration ritual and the governance metrics drew the same reaction, read as departures from what the coach's own organisation does today.

Two observations from the same session are reported as limitations rather than as validation, because neither produced a design change. The first concerns Apex's interface directly: navigating the tool mid-demonstration produced visible uncertainty about where a given action would lead next, and the coach named tooltips or contextual framing, and a tutorial, as what was missing. This is reported here rather than folded into \Cref{sec:apex_limitations}, because it was not evidenced against a specific defect the way the items in that section are; it is a first-contact usability observation from a single session, offered as an open gap rather than a measured finding. The second concerns the coach's own organisation rather than Apex: AI tool use there is centrally governed, restricted to a single permitted tool, and metered by token consumption, yet no role or specialist owns that governance, a gap large enough that the organisation is actively hiring for it. The coach raised, unprompted, whether AI participation in the lifecycle nets out as a cost or a benefit; the framework's governance metrics are positioned to answer exactly that question but do not yet operationalise it for token cost specifically, and the point is left here as a candidate for future work rather than pursued further in this dissertation.
