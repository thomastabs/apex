...
What that sample supports is bounded, and worth stating before any number is reported. Thirteen respondents meets the twelve at which a \ac{SUS} mean is generally treated as stable, so the mean is reported rather than suppressed, with the distribution and every individual score alongside it. It remains a convenience sample, skewed junior and assembled from a single network, and what that does and does not support is set out in full in \Cref{sec:threats}; no inferential statistic is computed anywhere in this chapter and no subgroup is compared against another.
...
unvalidated eighteen-item scale into a single figure would claim more than
the instrument supports. Administered last, after \ac{SUS}, and
...
with \Cref{tab:eval_instruments}: keeping them on the same sheet is
convenient for the participant, but splitting them out in analysis is what
