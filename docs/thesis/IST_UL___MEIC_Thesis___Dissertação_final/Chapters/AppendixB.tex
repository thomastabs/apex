...
rests on is stated in \Cref{sec:eval_participants}. There is no
recording-consent item, because no session and no interview was recorded.
...
\Cref{tab:sus_scores}; the workload matrix is reported as subscale means by
task in \Cref{tab:tlx_matrix} and the \ac{UX} questionnaire as full response
counts per item in \Cref{tab:ux_items}. The underlying per-participant
responses behind the latter two, anonymised and identified only by
participant code, are not printed here: a thirteen-participant by six-task by
six-subscale workload matrix and a thirteen by seventeen response matrix
would occupy more of this dissertation than their reporting value justifies.
They are retained by the researcher, as the consent form in
