...
\Cref{tab:eval_instruments}. The seventeen items below are those actually administered: a further item in construct A, on understanding a refusal at a phase gate, was removed from both language versions when task 8 was redesigned, for the reason given in \Cref{sec:apex_ux}.

\medskip
\noindent\begin{minipage}{\linewidth}
\captionof{table}{Apex UX questionnaire items, by construct, as administered}
\label{tab:appB_ux}
\centering
\small
\begin{tabular}{c p{12.6cm}}
...
Administered in Portuguese to participants selecting that language on G6,
using the direct translations in \texttt{docs/thesis/evaluation/instrument-apex-ux.md}: same seventeen items, same five constructs, same three open
