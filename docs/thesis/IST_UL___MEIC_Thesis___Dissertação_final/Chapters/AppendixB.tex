...
were administered; the eighteenth was removed before administration for the
reason given in \Cref{sec:apex_ux} and is not reproduced below.
...
\subsection*{Full item-by-item responses}

Response counts for all seventeen administered items, with the medians
\Cref{sec:results_ux} computes from them.
...
\Cref{tab:sus_scores} below; workload is reported as subscale means by task
in \Cref{tab:tlx_matrix} in \Cref{chap:evaluation}, and the \ac{UX}
questionnaire as full response counts per item in \Cref{tab:ux_items} above
(\Cref{sec:appB_ux}). The per-participant responses behind the latter two
are not printed (a thirteen by six by six workload matrix and a thirteen by
seventeen response matrix, out of proportion to their reporting value here).
