...
Completed first, before any task or questionnaire; the ethics position it
rests on is stated in \Cref{sec:eval_participants}.
...
\textbf{Supervisors:} Prof. Miguel Leitão Bignolas Mira da Silva, Hugo de Sousa.
...
\captionof{table}{Demographics items, collected on the consent form}
\label{tab:appB_demographics}
\centering
\small
...
\medskip
\noindent\begin{minipage}{\linewidth}
\captionof{table}{Raw NASA Task Load Index subscales, in the order presented}
\label{tab:appB_tlx}
\centering
\small
\begin{tabular}{>{\raggedright\arraybackslash}p{3.5cm} p{8.4cm} >{\raggedright\arraybackslash}p{2.4cm}}
\toprule
...
substituting \emph{Apex} for \emph{the system}. Participants selecting
Portuguese on G6 were given the European Portuguese version validated by
Martins et al.~\cite{Martins:2015SUS} verbatim, with the same system-name
substitution; that validation's psychometric limits, and what they mean for
the Portuguese figures, are reported in \Cref{sec:sus}.

\medskip
\noindent\begin{minipage}{\linewidth}
\captionof{table}{System Usability Scale items, as administered}
\label{tab:appB_sus}
\centering
\small
...
Not a validated instrument; its rationale and its five constructs are stated
in \Cref{sec:apex_ux}. Administered last, scale \texttt{1 = Strongly
disagree} \ldots{} \texttt{5 = Strongly agree} plus an explicit \emph{N/A -
did not use this}. Items E2 and E3 evaluate the framework rather than the
instantiation and are analysed separately from the rest. Seventeen items
were administered; an item originally written to pair against a guaranteed
task-8 refusal (\emph{when the tool refused to let me continue, I understood
why}) was removed from both language versions before administration once
that refusal trigger was redesigned out of task 8, for the reason given in
\Cref{sec:apex_ux}, and is not reproduced below.

\medskip
\noindent\begin{minipage}{\linewidth}
\captionof{table}{Apex UX questionnaire items, by construct}
\label{tab:appB_ux}
\centering
\small
\begin{tabular}{c p{12.6cm}}
...
Administered in Portuguese to participants selecting that language on G6,
using the direct translations in \texttt{docs/thesis/evaluation/instrument-apex-ux.md}: same items, same five constructs, same three open questions,
differing only in wording.
...
tool. Scheduled as a separate 30-45 minute session, not immediately after the
task session, and not recorded; the questions below are prompts for a
semi-structured conversation rather than a script read verbatim, and the
...
Every \ac{SUS} score is printed per participant, with both sub-scales, in
\Cref{tab:sus_scores}; workload is reported as subscale means by task in
\Cref{tab:tlx_matrix} and the \ac{UX} questionnaire as full response counts
per item in \Cref{tab:ux_items}. The per-participant responses behind the
latter two are not printed, a thirteen by six by six workload matrix and a
thirteen by seventeen response matrix being out of proportion to their
reporting value here. They are retained anonymised by the researcher, as
\Cref{sec:appB_consent} states, and are available from the author on
request.
