...
% 2026-09-13: the ulem={ULforem,...} option above makes \ULforem the active
% style immediately on load, which redefines \emph to underline instead of
% italicise - not just for \added{}/\replaced{} tracked-changes markup, but
% for every plain \emph{} in the document, from here through the whole thesis
% (main.tex's own \normalem/\ULforem toggle around \bibliography only ever
% flips it back for the bibliography itself, then flips it to underlined
% again for the appendices - chapters 1-10 never got \normalem at all). No
% \added{}/\replaced{} markup is actually used anywhere in the thesis, so
% restore normal italic emphasis right away; this is what was making every
% \emph{} (e.g. "Vibe Coding" in Chapter 1) render underlined.
\normalem
% commands:
% \added[id=xx]{text}
% \deleted[id=xx]{text}
...
% [1], [2], [5]--[7], [9]
% Commented out 2026-09-13 when main.tex briefly switched to apalike, then
% restored 2026-09-14 when the bibliography style reverted to IEEEtran
% (numeric) - see main.tex's bibliographystyle comment.
\usepackage{cite}
%
% -----------------------------------------------------------------------------
% PACKAGE acronym:
...
% tocdepth was 5 (paragraph level), which pulled paragraph-level headings like
% Chapter 9's "A - Practitioners." into the table of contents as their own
% clutter rows. 3 = subsubsection, so entries like "4.3.1 Benefits of AI..."
% still show; only the paragraph level below them is dropped from the TOC.
% secnumdepth is left at 5 on purpose - this only changes what the TOC lists,
% not what gets numbered in the body.
